...
export const isIntegrationBlock = (block: Block): block is IntegrationBlock => {
  const a = Object.values(IntegrationBlockType)
  const b = Object.values(MilvusBlockType)
  const c = enabledBlocks as readonly any[]

  const total: any[] = [...a, ...b, ...c]

  return total.includes(block.type)
}
...
export const isWebhookBlock = (block: Block): block is HttpRequestBlock =>
  [
    ...
    MilvusBlockType.CREATE_TICKET,
    MilvusBlockType.BASE_BY_TAG,
    MilvusBlockType.ARTICLE_BY_ID,
    MilvusBlockType.CLIENT_BY_DOCUMENTO,
    MilvusBlockType.CLIENT_BY_EMAIL,
